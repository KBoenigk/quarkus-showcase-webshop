...
@DataSet(value = "webshop.yml", strategy = SeedStrategy.CLEAN_INSERT, skipCleaningFor = "flyway_schema_history")
  ...
  private static final GenericContainer<?> NEWMAN = new GenericContainer<>("postman/newman:5.1.0-alpine")
      .withCopyFileToContainer(MountableFile.forClasspathResource("postman/Webshop.postman_collection.json"),
                               "/etc/newman/Webshop.postman_collection.json")
      .withCopyFileToContainer(MountableFile.forClasspathResource("postman/Webshop.postman_environment.json"),
                               "/etc/newman/Webshop.postman_environment.json")
  ...
  @Test
  void run() {
    NEWMAN.withCommand("run", "Webshop.postman_collection.json",
                       "--environment=Webshop.postman_environment.json",
                       "--reporters=cli,junit",
                       "--reporter-junit-export=reports/Webshop.newman-report.xml");
    NEWMAN.start();
